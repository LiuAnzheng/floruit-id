...
public class BizHandler extends SimpleChannelInboundHandler<ReqData> {
    ...
    private static final Disruptor<AbstractEvent> snowflakeDisruptor = DisruptorHolder.getSnowflakeDisruptor();
    private static final Disruptor<AbstractEvent> segmentDisruptor = DisruptorHolder.getSegmentDisruptor();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, ReqData msg) throws Exception {
        ...
        if (IdMode.SNOW_FLAKE.getMode().equals(mode) && config.getOpenSnowFlakeMode()) {
            // 雪花算法
            snowflakeDisruptor.publishEvent((event, sequence) -> {
            ...
            // 号段模式
            segmentDisruptor.publishEvent((event, sequence) -> {
                DefaultEvent e = (DefaultEvent) event;
                e.setCtx(ctx);
                e.setKey(msg.getKey());
                e.setReqId(msg.getReqId());
